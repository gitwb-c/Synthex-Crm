<script lang="ts">
  ...
  const submit = async () => {
    ...
    const res = await Helper.fetchUrl(
      `${PUBLIC_BACKEND_API_URL}/auth/login/${params.companyId}`,
    ...
    if (res.statusCode == 200) {
      const query = `
        ...
        `;
      const res2 = await Helper.fetchUrl(
        `${PUBLIC_BACKEND_API_URL}/graphql/query/${params.companyId}`,
        { method: "POST", body: JSON.stringify({ query }) }
      );
      goto(`/company/${params.companyId}/home`);
    } else {
      alert("OAL");
