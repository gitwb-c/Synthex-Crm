...
func (_q *FileQuery) QueryMessage() *MessageQuery {
...
func (_q *FileQuery) WithMessage(opts ...func(*MessageQuery)) *FileQuery {
	query := (&MessageClient{config: _q.config}).Query()
...
func (_q *FileQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*File, init func(*File), assign func(*File, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.MessageColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.file_message
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_message" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_message" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
