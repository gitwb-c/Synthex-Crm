...
func (_q *DepartmentQuery) QueryEmployee() *EmployeeQuery {
...
func (_q *DepartmentQuery) QueryQueues() *QueueQuery {
...
func (_q *DepartmentQuery) WithEmployee(opts ...func(*EmployeeQuery)) *DepartmentQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
...
func (_q *DepartmentQuery) WithQueues(opts ...func(*QueueQuery)) *DepartmentQuery {
	query := (&QueueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := _q.withEmployee; query != nil {
		if err := _q.loadEmployee(ctx, query, nodes,
			func(n *Department) { n.Edges.Employee = []*Employee{} },
			func(n *Department, e *Employee) { n.Edges.Employee = append(n.Edges.Employee, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withQueues; query != nil {
		if err := _q.loadQueues(ctx, query, nodes,
			func(n *Department) { n.Edges.Queues = []*Queue{} },
			func(n *Department, e *Queue) { n.Edges.Queues = append(n.Edges.Queues, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedEmployee {
		if err := _q.loadEmployee(ctx, query, nodes,
			...
			func(n *Department, e *Employee) { n.appendNamedEmployee(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *DepartmentQuery) loadEmployee(ctx context.Context, query *EmployeeQuery, nodes []*Department, init func(*Department), assign func(*Department, *Employee)) error {
	...
	query.Where(predicate.Employee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.EmployeeColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.employee_department
		if fk == nil {
			return fmt.Errorf(`foreign-key "employee_department" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "employee_department" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *DepartmentQuery) loadQueues(ctx context.Context, query *QueueQuery, nodes []*Department, init func(*Department), assign func(*Department, *Queue)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Department)
	nids := make(map[uuid.UUID]map[*Department]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(department.QueuesTable)
		s.Join(joinT).On(s.C(queue.FieldID), joinT.C(department.QueuesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(department.QueuesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(department.QueuesPrimaryKey[1]))
...
func (_q *DepartmentQuery) WithNamedEmployee(name string, opts ...func(*EmployeeQuery)) *DepartmentQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *DepartmentQuery) WithNamedQueues(name string, opts ...func(*QueueQuery)) *DepartmentQuery {
	query := (&QueueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
