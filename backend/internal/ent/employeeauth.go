...
// was not loaded in eager-loading, or loaded but was not found.
func (e EmployeeAuthEdges) TenantOrErr() (*Company, error) {
	if e.Tenant != nil {
		return e.Tenant, nil
...
func (*EmployeeAuth) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case employeeauth.FieldID, employeeauth.FieldTenantId:
...
// QueryTenant queries the "tenant" edge of the EmployeeAuth entity.
func (_m *EmployeeAuth) QueryTenant() *CompanyQuery {
	return NewEmployeeAuthClient(_m.config).QueryTenant(_m)
}
