...
func (_q *ChatQuery) QueryDeal() *DealQuery {
...
func (_q *ChatQuery) QueryEmployees() *EmployeeQuery {
...
func (_q *ChatQuery) WithDeal(opts ...func(*DealQuery)) *ChatQuery {
	query := (&DealClient{config: _q.config}).Query()
...
func (_q *ChatQuery) WithEmployees(opts ...func(*EmployeeQuery)) *ChatQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := _q.withDeal; query != nil {
		if err := _q.loadDeal(ctx, query, nodes, nil,
			func(n *Chat, e *Deal) { n.Edges.Deal = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEmployees; query != nil {
		if err := _q.loadEmployees(ctx, query, nodes,
			func(n *Chat) { n.Edges.Employees = []*Employee{} },
			func(n *Chat, e *Employee) { n.Edges.Employees = append(n.Edges.Employees, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedEmployees {
		if err := _q.loadEmployees(ctx, query, nodes,
			func(n *Chat) { n.appendNamedEmployees(name) },
			func(n *Chat, e *Employee) { n.appendNamedEmployees(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedMessages {
		if err := _q.loadMessages(ctx, query, nodes,
			func(n *Chat) { n.appendNamedMessages(name) },
			func(n *Chat, e *Message) { n.appendNamedMessages(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ChatQuery) loadDeal(ctx context.Context, query *DealQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Deal)) error {
	...
	query.Where(predicate.Deal(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.DealColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.chat_deal
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_deal" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_deal" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ChatQuery) loadEmployees(ctx context.Context, query *EmployeeQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Employee)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(chat.EmployeesTable)
		s.Join(joinT).On(s.C(employee.FieldID), joinT.C(chat.EmployeesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(chat.EmployeesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(chat.EmployeesPrimaryKey[0]))
...
func (_q *ChatQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Message)) error {
	...
	for _, n := range neighbors {
		fk := n.message_chat
		if fk == nil {
			return fmt.Errorf(`foreign-key "message_chat" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "message_chat" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ChatQuery) WithNamedEmployees(name string, opts ...func(*EmployeeQuery)) *ChatQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatQuery) WithNamedMessages(name string, opts ...func(*MessageQuery)) *ChatQuery {
	query := (&MessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
