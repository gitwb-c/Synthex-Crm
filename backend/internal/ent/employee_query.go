...
type EmployeeQuery struct {
	...
	withEmployeeAuth  *EmployeeAuthQuery
	...
	withNamedChat     map[string]*ChatQuery
...
// QueryEmployeeAuth chains the current query on the "employeeAuth" edge.
func (_q *EmployeeQuery) QueryEmployeeAuth() *EmployeeAuthQuery {
...
func (_q *EmployeeQuery) QueryCompany() *CompanyQuery {
...
func (_q *EmployeeQuery) QueryChat() *ChatQuery {
...
func (_q *EmployeeQuery) QueryQueues() *QueueQuery {
...
func (_q *EmployeeQuery) QueryMessages() *MessageQuery {
...
func (_q *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withEmployeeAuth: _q.withEmployeeAuth.Clone(),
...
// WithEmployeeAuth tells the query-builder to eager-load the nodes that are connected to
// the "employeeAuth" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EmployeeQuery) WithEmployeeAuth(opts ...func(*EmployeeAuthQuery)) *EmployeeQuery {
	query := (&EmployeeAuthClient{config: _q.config}).Query()
...
func (_q *EmployeeQuery) WithCompany(opts ...func(*CompanyQuery)) *EmployeeQuery {
	query := (&CompanyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmployeeQuery) WithChat(opts ...func(*ChatQuery)) *EmployeeQuery {
	query := (&ChatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmployeeQuery) WithQueues(opts ...func(*QueueQuery)) *EmployeeQuery {
	query := (&QueueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmployeeQuery) WithMessages(opts ...func(*MessageQuery)) *EmployeeQuery {
	query := (&MessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := _q.withEmployeeAuth; query != nil {
		if err := _q.loadEmployeeAuth(ctx, query, nodes, nil,
			func(n *Employee, e *EmployeeAuth) { n.Edges.EmployeeAuth = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withChat; query != nil {
		if err := _q.loadChat(ctx, query, nodes,
			func(n *Employee) { n.Edges.Chat = []*Chat{} },
			func(n *Employee, e *Chat) { n.Edges.Chat = append(n.Edges.Chat, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withQueues; query != nil {
		if err := _q.loadQueues(ctx, query, nodes,
			func(n *Employee) { n.Edges.Queues = []*Queue{} },
			func(n *Employee, e *Queue) { n.Edges.Queues = append(n.Edges.Queues, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMessages; query != nil {
		if err := _q.loadMessages(ctx, query, nodes,
			func(n *Employee) { n.Edges.Messages = []*Message{} },
			func(n *Employee, e *Message) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedChat {
		if err := _q.loadChat(ctx, query, nodes,
			func(n *Employee) { n.appendNamedChat(name) },
			func(n *Employee, e *Chat) { n.appendNamedChat(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedQueues {
		if err := _q.loadQueues(ctx, query, nodes,
			func(n *Employee) { n.appendNamedQueues(name) },
			func(n *Employee, e *Queue) { n.appendNamedQueues(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedMessages {
		if err := _q.loadMessages(ctx, query, nodes,
			func(n *Employee) { n.appendNamedMessages(name) },
			func(n *Employee, e *Message) { n.appendNamedMessages(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *EmployeeQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Company)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Employee)
	for i := range nodes {
		if nodes[i].employee_company == nil {
			continue
		}
		fk := *nodes[i].employee_company
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "employee_company" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EmployeeQuery) loadDepartment(ctx context.Context, query *DepartmentQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Department)) error {
	...
	nodeids := make(map[uuid.UUID][]*Employee)
	for i := range nodes {
		if nodes[i].employee_department == nil {
			continue
		}
		fk := *nodes[i].employee_department
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "employee_department" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EmployeeQuery) loadChat(ctx context.Context, query *ChatQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Chat)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(employee.ChatTable)
		s.Join(joinT).On(s.C(chat.FieldID), joinT.C(employee.ChatPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(employee.ChatPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(employee.ChatPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "chat" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *EmployeeQuery) loadQueues(ctx context.Context, query *QueueQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Queue)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Employee)
	nids := make(map[uuid.UUID]map[*Employee]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(employee.QueuesTable)
		s.Join(joinT).On(s.C(queue.FieldID), joinT.C(employee.QueuesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(employee.QueuesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(employee.QueuesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *EmployeeQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Message)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Employee)
	nids := make(map[uuid.UUID]map[*Employee]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(employee.MessagesTable)
		s.Join(joinT).On(s.C(message.FieldID), joinT.C(employee.MessagesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(employee.MessagesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(employee.MessagesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
// WithNamedChat tells the query-builder to eager-load the nodes that are connected to the "chat"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *EmployeeQuery) WithNamedChat(name string, opts ...func(*ChatQuery)) *EmployeeQuery {
	query := (&ChatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedChat == nil {
		_q.withNamedChat = make(map[string]*ChatQuery)
	}
	_q.withNamedChat[name] = query
	return _q
}

// WithNamedQueues tells the query-builder to eager-load the nodes that are connected to the "queues"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *EmployeeQuery) WithNamedQueues(name string, opts ...func(*QueueQuery)) *EmployeeQuery {
	query := (&QueueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *EmployeeQuery) WithNamedMessages(name string, opts ...func(*MessageQuery)) *EmployeeQuery {
	query := (&MessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
